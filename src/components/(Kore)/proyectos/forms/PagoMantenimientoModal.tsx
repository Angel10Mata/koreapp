...
import { Proyecto, MantenimientoRecord } from "@/components/(Kore)/proyectos/lib/zod";
...
const getMaintenanceBaseStartDate = (proyecto: Proyecto, paymentsList: MantenimientoRecord[]) => {
...
const calculateNextCobroDate = (baseStartDate: Date | null, paidPeriods: string[]) => {
  ...
  const current = new Date(baseStartDate);
...
export function PagoMantenimientoModal({ proyecto, onClose, onSuccess }: PagoMantenimientoModalProps) {
  ...
  const [payments, setPayments] = useState<MantenimientoRecord[]>([]);
